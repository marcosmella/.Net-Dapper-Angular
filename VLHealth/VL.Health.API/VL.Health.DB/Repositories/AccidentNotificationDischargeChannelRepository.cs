...
{
    public class AccidentNotificationDischargeChannelRepository	: IAccidentNotificationDischargeChannelRepository
    {
		private readonly string _connectionString;

		public AccidentNotificationDischargeChannelRepository(ITenantResolver tenantResolver, IHttpContextAccessor httpContextAccessor)
		{
		...
		public List<AccidentNotificationDischargeChannel> Get()
		{
			using (var connection = new SqlConnection(_connectionString))
			{
				var sql = @$"SELECT
					...
					where soaccidente_valores_control.idcontrol =  {(int)AccidentValueControl.NotificationDischargeChannels}";

				return connection.Query<AccidentNotificationDischargeChannel>(sql).AsList();
			}
		}
	}
}
